...
def transcript_system(pre_prompt: str) -> str:
    return (
        ...
        "其中可能带有「发言人1」「主持人：」「Speaker 2」等说话人标签。\n"
        "请在【尽量保留原话】的前提下完成：\n"
        ...
        "3) 去时间戳：删除文中的时间戳（如「00:01:23」「[12:30]」「(1:23)」「03:15 -->」等），但【保留说话人标签】；\n"
        "4) 按发言人排版：\n"
        "   - 每位说话人的发言以【加粗标签】开头，形如 `**发言人1：**`，紧跟其发言正文；\n"
        "   - 不同说话人之间【空一行】，形成明显区隔；\n"
        "   - 同一位说话人连续的发言【合并成一个完整段落，不要拆成多段、不要打断】；\n"
        "   - 若本片段开头没有说话人标签（属上一位说话人发言的延续），则【不要自行编造说话人】，"
        "直接顺接整理正文即可；若整段都没有任何说话人标签，则输出连贯的纯正文、不要杜撰发言人。\n\n"
...
def chapters_system(pre_prompt: str) -> str:
    return (
        "你是课堂内容结构分析师。基于给定的课堂内容，按【时间顺序】梳理【整堂课】的演讲逻辑与阶段。\n"
        "输出一份【连续、完整、覆盖全程】的阶段列表，用 Markdown，每个阶段格式如下：\n\n"
        "### 阶段N：阶段主题（时间区间）\n"
        "（2-4 句话概括该阶段核心内容）\n\n"
        "【硬性要求】\n"
        "1) 阶段编号从 1 开始【连续递增、贯穿整堂课】，绝不分段重新计数；整堂课通常划分为 5-10 个阶段。\n"
        "2) 阶段划分体现讲者的逻辑推进，前后自然衔接；不要把同一主题切成多个碎片阶段，"
        "若发现内容有重复或承接，请合并成一个连贯的阶段。\n"
        "3) 时间区间【只依据下方「时间轴参考」标注】：根据每个阶段的内容，在时间轴参考里找到对应片段的时间戳，"
        "推断该阶段的起止时间，形如 (00:00–12:30)。若未提供时间轴参考或无法对应，则时间区间留空括号 ()。\n"
        "只输出阶段列表，不要任何前后说明。"
        + _bg(pre_prompt)
    )


def chapters_user(body: str, timeline_digest: str = "") -> str:
    parts = [f"【课堂内容】\n{body}"]
    if (timeline_digest or "").strip():
        parts.append(
            "【时间轴参考】（来自原始转写稿的时间戳 + 片段开头，仅用于为各阶段对位标注时间区间；"
            "其中文字可能含口误或识别错误，不要作为内容依据）\n" + timeline_digest
        )
    else:
        parts.append("【时间轴参考】无（原始稿未含时间戳，时间区间一律留空括号）")
    return "\n\n".join(parts)
