...
**v0.8.0** · 多场景会议纪要生成平台（本地优先）。当前开放：**商学院课堂讲座**板块。

- 方案与架构：[docs/开发文档_v0.8.0.md](docs/开发文档_v0.8.0.md)（最新版；历史版本保留在 docs/）
...
| 产出 | 说明 |
|---|---|
| 实录 | 纠错后的逐字稿（口水词清理、专业词保留、时间锚点小标题；无法理解处标 `[听不清]`、事实拿不准标 `[存疑]`） |
| 纲目 | 章节结构稿（部分级篇章分组 + 按课堂长度动态收敛的阶段，时间区间从原文时间戳对位） |
| 撷要 | 精炼版纪要（七节结构含中英术语表；"对你的启发"基于个人画像个性化） |
| 笺注 | 深度笺注（按纲目逐阶段展开：核心论点、论证链、例证逐字摘引、现场问答；完整收录不注水） |
| 脉络 | Mermaid 知识图谱（连通性校验保证课程主线成图；矢量可缩放） |

每个产出可**独立生成**、**独立选模型**、**持续迭代修订**（按意见再生成新版本、保留版本历史、可恢复旧版），并支持单个产出下载与项目产出 ZIP 打包导出。支持 Claude、DeepSeek、自定义 OpenAI 兼容端点。

另有：**个人画像**（全局 + 项目两级，个性化"对你的启发"）；**笔记照片校对**（识别结果可查看/编辑/重试，人工校对优先生效）。
