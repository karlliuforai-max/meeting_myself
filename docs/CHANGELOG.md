...
### 整体加固：对抗性审查发现的 17+6 项问题全部修复

**安全**
- 修复上传文件名与 step 参数的路径穿越（文件名净化为 basename；step 全入口白名单 + runner 正则防御）
- API Key 明文不再回传：详情接口只给掩码 + has_key；编辑留空保留原 key（patch 语义）；连通测试可借用已存 key
- Markdown 渲染加 DOMPurify 消毒；mermaid securityLevel loose → strict

**稳定性**
- providers.json 原子写；解析失败备份为 .corrupt-<ts> 再播种，不再静默重置用户配置
- meta.json 前向兼容：未知字段忽略、坏文件跳过，单个坏会话不再拖垮会话列表
- 服务重启自愈：启动时为无终态的进行中任务补写中断 error；前端 EventSource onerror 兜底，不再永远「生成中…」
- SSE 改 async 非阻塞轮询，不再每订阅独占一个线程池线程
- Provider client 复用 + 显式 180s 超时；进度文件原子写

**质量**
- 修订输出上限不再写死 4096：按步骤复用生成时上限（修订实录/笺注不再被静默截断）
- 全链路截断检测：finish_reason=length 显式报错，不落盘残缺产出（实录纠错块走既有重试/兜底）
- 文本输入编码探测：utf-8-sig → gb18030 → 替换兜底（GBK 转写稿不再变乱码）
- 纲目阶段数绝对下限 20 → 4（目标仍按课堂时长动态收敛），短课堂不再被摊碎
- 多文件转写稿逐文件估时长再求和
- 供应商新增 max_output_tokens 配置，请求自动收敛到模型能力内

**速度与体验**
- 撷要/笺注/修订/纲目汇编走 SDK 流式，生成期实时显示「已生成约 N 字…」（瞬态事件不落盘；不支持流式的中转站自动降级）
- 产出存在性检查不再整读文件内容；会话详情下发每步生效模型（resolved_step_models），前端删除复刻的默认解析逻辑
- 上传类型对齐：去掉后端不处理的 .pdf，补 .gif/.bmp

**测试**
- 新增 test_hardening.py 22 项；pytest 42 项全绿；真实冒烟覆盖穿越/GBK/SSE 流式/重启标记全链路
