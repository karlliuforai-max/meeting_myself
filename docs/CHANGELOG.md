...
### 产出内容质量升级：深度笺注 + 五产出全面提质（对抗性审查驱动，真实课堂端到端验证）

**深度笺注（重构）**
- 笺注从"字体稍大的撷要"重构为按纲目逐阶段展开的深度复盘：单元切片并行深潜 → 覆盖率闸门 → 确定性拼装 + 导读
- 反膨胀机制写入提示词：证据绑定/完整收录承诺/套话禁令/空节即省/摘引逐字——长度由内容密度决定，无字数指标
- 真实 3 小时课堂验证：4.8K → 28K 字，26 阶段全过闸门、161 处逐字摘引，并能主动标注转写疑点（发言人错位）供人工复核
- 依赖变更：笺注 requires 实录 + 纲目；"对你的启发"与术语表专属撷要，两产出真正分化

**实录**
- 可读性优先分级红线：能修则修 / 完全无法理解标 `[听不清]` / 事实拿不准标 `[存疑：原文]`（不再死守"保留原文"）
- 分块携带前块结尾作衔接上下文（缓解问答压平与跨块标签漂移）；确定性时间锚点小标题 `#### ⏱ H:MM:SS`

**纲目 / 脉络**
- 纲目新增 `## 第N部分` 篇章分组（3-6 个宏观部分）
- 脉络连通性硬约束 + graphcheck 确定性校验：孤岛图带反馈返修一次，仍不过报错不落盘（修订同样校验）；输入增加纲目授课顺序

**个人画像**
- 三级优先级：项目 > 全局 > 系统默认；顶栏全局编辑 + 工作台项目级编辑；注入撷要/笺注/修订，"对你的启发"基于真实画像不再虚构身份

**笔记照片**
- 失败原因逐张透传 + 自动重试；转录可见可校对（notes API + 前端面板：查看/编辑/保存校对/恢复自动/重新识别）；用户校对优先且不随图片改动失效
- 修复真实根因：扩展名与内容不符（.jpg 实为 PNG）按魔数嗅探 media_type；补装 Pillow

**真实验证中修复的兼容性问题**
- 新模型经中转站拒绝 temperature → provider 自动去参重试并记忆
- 思考型模型隐藏思考计入 max_tokens → 小额上限整体上调（导读 4000/撷要 8000/脉络 8000/笺注单元 16000）
- 中转站偶发丢 system prompt 导致导读反问 → 指令写进 user 消息 + 引擎侧合理性校验（不合格重试→省略，绝不放坏内容）
- 中转站连接抖动 → 单元瞬时故障重试（截断类直接上抛）

**测试**
- pytest 42 → 97 项全绿（新增 deepnotes/graphcheck/transcript/persona_notes/temperature 重试）；前端 build 通过
